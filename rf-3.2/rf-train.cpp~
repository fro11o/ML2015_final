...
int dt_dfs(int L, int R) {
	...
	for(int i = 0; i < X[0].size(); ++i) {
		...
		for(int j = 0; j < pos_cnt.size(); ++j) {  //ignore 0 for avoid devide by 0
			double gini[2];
...
double weighted_tot = 0;
vector<double> weighted_seq;
void weighted_sample_init() {
	for(int i = 0; i < D.size(); ++i) {
		weighted_tot += 1.0 / X[i][2];
		weighted_seq.push_back(weighted_tot);
	}
}

int weighted_sample() {
	double res = 1. * rand() / RAND_MAX * weighted_tot;
	int idx = std::lower_bound(weighted_seq.begin(), weighted_seq.end(), res) - weighted_seq.begin();
	cout << "res " << res << " idx " << idx << endl;
	return idx;
}

int main(int argc, char *argv[]) {
	if(argc != 4) {
        cout << "usage: ./a.out [train.dat] [# of tree] [(N' / N) * 100%]" << endl;
		return 0;
	}
