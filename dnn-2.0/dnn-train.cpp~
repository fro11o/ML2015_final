...
void nn_solve(double lrate, int n) {
	...
	for(int i = 0; i < n; ++i) {
		...
		for(int j = AUTO_DIM.size() - 1; j > 0; --j) {  //j for layer
			for(int k = 1; k < AUTO_DIM[j - 1]; ++k) {
				for(int l = 1; l < AUTO_DIM[j]; ++l) {
					//calculate d error / d w_kl

					double W_X = 0;
					for(int m = 1; m < AUTO_DIM[j - 2]; ++m) {
						W_X += W[j - 1][m][k] * AUTO_NN_X[j - 2][m];
					}

					for(int m = 1; m < AUTO_DIM[j - 1]; ++m) {
						double res = 2 * (AUTO_NN_X[j][m] - AUTO_NN_X[j - 2][m]);
						double res2 = W[j][k][m] * tanhh(W_X) * AUTO_NN_X[j - 2][l];
						if(m == l) {
							res2 += tanh(W_X);
						}
						AUTO_TMP_W[j][k][l] += res * res2;
					}
				}
			}
