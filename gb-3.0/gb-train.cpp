...
int dt_dfs(int L, int R, int height) {
	...
	for(int i = 0; i < X[0].size(); ++i) {
		...
		vector<double> sum;
		vector<double> square_sum;
		sum.push_back(0);
		square_sum.push_back(0);
		...
		for(int j = L; j < R; ++j) {
			...
			while(j < R && fabs(x_y_idx[j].first - x_y_idx[idx].first) < eps) {
				...
				tmp_sum2 += pow((x_y_idx[j].second).first, 2);
				++j;
			}
			--j;
			sum.push_back(sum[sum.size() - 1] + tmp_sum);
			square_sum.push_back(square_sum[square_sum.size() - 1] + tmp_sum2);
		...
		for(int j = 0; j < cut_location.size() - 1; ++j) {  //ignore the last one
			...
			double err[2] = {0, 0};
			err[0] = square_sum[j] / (cut - L) - pow(avg[0], 2);
			err[1] = (square_sum[square_sum.size() - 1] - square_sum[j]) / (R - cut) - pow(avg[1], 2);
...
void gb(int T, string s, int height, int NN) {

	s_init();

	for(int t = 0; t < T; ++t) {

		for(int i = 0; i < D.size() * NN / 100; ++i) {
			int idx = rand() % D.size();
			x_y_idx[i] = make_pair(0, make_pair(Y[idx] - S[idx], idx));
		}

		v_node.clear();

		dt_dfs(0, D.size() * NN / 100, height);
...
int main(int argc, char *argv[]) {
	if(argc != 5) {
        cout << "usage: ./a.out [train.dat] [# of iter] [max height] [(N' / N) * 100%]" << endl;
		return 0;
	}
	...
	gb(stoi(argv[2]), argv[1], stoi(argv[3]), stoi(argv[4]));
