...
void nn_solve(double lrate, int n) {
	...
	for(int i = 0; i < n; ++i) {
		...
		for(int j = 1; j < DIM[0]; ++j) {
			NN_X[0][j] = 1.0 * X[idx][j - 1] - MIN[j - 1];
			if(SCALE[j - 1] > eps) {
				NN_X[0][j] /= SCALE[j - 1];
			}
		}
		...
		for(int j = DIM.size() - 1; j > 0; --j) {  //j for layer
			for(int k = 1; k < DIM[j]; ++k) {  //k for dimension in j-layer
				if(j == DIM.size() - 1) {
					DELTA[j][k] = -2.0 * (Y[idx] - tanh(NN_X[j][k])) * tanhh(NN_X[j][k]);
...
int main(int argc, char *argv[]) {
	if(argc <= 5) {
		cout << "usage nn-train [train_dat] [# of iter] [N' each time o/oo] [lrate] [<# of 1 layer>, <# of 2 layer>, ...]" << endl;
		return 0;
	}
	...
	for(int iter = 0; iter < stoi(argv[2]); ++iter) {
		nn_solve(stod(argv[4]), D.size() * stoi(argv[3]) / 1000);
	}
