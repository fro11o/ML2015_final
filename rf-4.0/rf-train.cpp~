...
int dt_dfs(int L, int R, int h) {
	...
	if(leaf_check == L || leaf_check == R) {
		...
		if(cur_node.s == -1) {
			cur_node.ch[1] = dt_dfs(L, leaf_check, h - 1);
			cur_node.ch[0] = dt_dfs(leaf_check, R, h - 1);
		} else {
			cur_node.ch[0] = dt_dfs(L, leaf_check, h - 1);
			cur_node.ch[1] = dt_dfs(leaf_check, R, h - 1);
		}
	}
	v_node.push_back(cur_node);
	return v_node.size() - 1;
...
int main(int argc, char *argv[]) {
	if(argc != 5) {
        cout << "usage: ./a.out [train.dat] [# of tree] [tree height] [(N' / N) * 100%]" << endl;
		return 0;
	}
	...
	//print_data();	
	for(int i = 0; i < D.size() * stoi(argv[4]) / 100; ++i) {
	...
	for(int t = 0; t < stoi(argv[2]); ++t) {
		...
		dt_dfs(0, D.size() * stoi(argv[4]) / 100, stoi(argv[3]));
		write_model(argv[1]);
		v_node.clear();
	}
